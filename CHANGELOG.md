...
- **Exploit intelligence contract hardening**: Added stage-output validation for `exploit_intel.json` and `autopoc_seeds.json`, and tightened Aqua vuln-list/NVD normalization so only explicitly vulnerable CPE matches enter metadata-only AutoPoC seeds.
